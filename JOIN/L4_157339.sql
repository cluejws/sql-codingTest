...
SELECT C.CAR_ID AS 'CAR_ID', 
    ...
    ROUND(DAILY_FEE * ((100 - P.DISCOUNT_RATE) / 100) * 30, 0) AS 'FEE'
FROM CAR_RENTAL_COMPANY_CAR C JOIN CAR_RENTAL_COMPANY_DISCOUNT_PLAN P ON (C.CAR_TYPE = P.CAR_TYPE)
WHERE C.CAR_TYPE IN ('세단', 'SUV')
    AND C.CAR_ID NOT IN (SELECT NH.CAR_ID FROM NOV_HISTORY_TABLE NH)
    AND P.DURATION_TYPE = '30일 이상'
    AND DAILY_FEE * ((100 - P.DISCOUNT_RATE) / 100) * 30 >= 500000
    AND DAILY_FEE * ((100 - P.DISCOUNT_RATE) / 100) * 30 < 2000000
